...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-cb9c41a6";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-cb9c41a6">'+'<span class="tile-face-cb9c41a6 tile-front-cb9c41a6"></span>'+'<span class="tile-face-cb9c41a6 tile-back-cb9c41a6"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-cb9c41a6")){return;}
var back=tile.querySelector(".tile-back-cb9c41a6");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-cb9c41a6");if(isMine){tile.classList.add("boom-cb9c41a6");}
if(dim){tile.classList.add("dimmed-cb9c41a6");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-cb9c41a6")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-cb9c41a6");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-cb9c41a6")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-cb9c41a6");void panelEl.offsetWidth;panelEl.classList.add("jolt-cb9c41a6");}
endRound();}
